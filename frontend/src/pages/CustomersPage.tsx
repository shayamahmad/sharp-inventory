...
import { Search, Plus, X, Users, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function CustomersPage() {
  ...
  const customerStats = useMemo(() => customers.map(c => {
    ...
    return { ...c, totalOrders, lifetimeValue, avgOrderValue, lastOrder, custOrders };
  }).filter(c => c.name.toLowerCase().includes(search.toLowerCase()) || c.email.toLowerCase().includes(search.toLowerCase())), [customers, orders, search]);
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="stat-card">
          ...
          <p className="text-2xl font-display font-bold text-foreground">{formatCurrency(customerStats.reduce((s, c) => s + c.lifetimeValue, 0))}</p>
        </div>
        <div className="stat-card">
          <p className="text-xs text-muted-foreground">Avg Order Value</p>
          <p className="text-2xl font-display font-bold text-foreground">{formatCurrency(customerStats.reduce((s, c) => s + c.avgOrderValue, 0) / Math.max(customerStats.length, 1))}</p>
        </div>
        <div className="stat-card">
          <p className="text-xs text-muted-foreground">Repeat Customers</p>
          <p className="text-2xl font-display font-bold text-foreground">{customerStats.filter(c => c.totalOrders > 1).length}</p>
        </div>
      </div>
